...
| **v17** | 8 Jul 2026 | **ORE + GAP index engines — BENCH (Knowledge Version #2)**. Built two index-native continuation engines (ORE = opening-range breakout, GAP = overnight gap-and-go), TDD logic modules (ore_logic/gap_logic, suite 46→63). Probe findings pre-empted the design: these index CFDs have NO detectable cash-open (tick activity tracks the broker's US-afternoon client peak, not home opens; AUS200 zero data at its open) and near-zero overnight gaps (median 0.03 ATR) — so empirical open-detection was abandoned for fixed cash-open times and AUS200 dropped. Index spread counted REALISTICALLY (GVE-style, not the FX profiles' 100×-under-count). **Result: both FAIL, zero promotions.** ORE 6,346 trades PF 0.96 net −RM690 (index PF 0.91, FX PF 0.96 — cross-test invariant confirmed: fails on FX too). GAP under-sampled (CFDs barely gap). Finding: event-based engines don't fit continuous index CFDs — the index opportunity needs continuous-stream engines (VWAP/trend/dip-buy) or different data. Also this session: gold-crosses/oil/copper confirmed absent or dataless on this broker (FX-centric demo); Windows Defender quarantined the Python install mid-run (stdlib+extensions+packages) — repaired via embeddable-package restore + pip reinstall, live system healthy. Engines stay flag-gated OFF; logic modules + index pipeline retained. | — | — | — | — | — | — | git tag `v17` |

| **v18** | 8 Jul 2026 | **Lower-Timeframe Track — CBE15 + IRE15 (isolated, additive)**. Ran CBE + IRE on M15 as SEPARATE engine labels (current H1/H4 system frozen — Mahesh's hard constraint), realistic spread (100× the FX profiles' under-count). IRE15: aggregate loses to spread (PF 0.94) but elite-narrow — 2 combos OOS-validated. CBE15: aggregate net-POSITIVE (PF 1.13, +RM3,701) — 4 combos OOS-validated, incl. the frequency breakthrough CBE15×EURUSD (365 OOS trades, +RM1,403, 7/8 folds). **Promoted 6 combos**: CBE15 × {EURUSD, USDCAD, USDJPY, NZDJPY} + IRE15 × {AUDUSD, USDJPY} — all matrix-PASS + OOS majority-fold-positive, on 0.5× probation. Held: IRE15×EURCHF (4/7 folds). NZDJPY added to universe. Finding: low-TF isn't a blanket multiplier (only 6 of 56 combos beat spread) but CBE15's survivors are high-frequency AND high-quality — roughly doubles live trade frequency. New live methods `_engine_cbe15`/`_engine_ire15` (M15 already fetched live). Also this session: **critical SIM bug fixed** — SIM never held positions (dummy ticket 99999, read real MT5 positions) so every trade closed BE instantly; built a virtual-position engine (holds to SL/TP, correct pnl). test_full_system 24/24. | +6 combos | — | — | — | — | — | git tag `v18` |

### v18 promotions (config.py — untracked; recorded here per the config-is-never-committed rule)
Lower-TF combos added to `KIRA_ROUTING_TABLE` (appended, existing engines keep priority), `ENGINE_SYMBOL_WHITELIST` (new CBE15/IRE15 keys), `JPY_PAIRS` (+NZDJPY), `PROBATION_COMBOS`:
- **CBE15 × EURUSD** — matrix PF 1.67, OOS +RM1,403 (7/8 folds, 365 trades). Frequency+profit star.
- **CBE15 × USDCAD** — matrix PF 1.30, OOS +RM586 (6/8 folds, 377 trades).
- **CBE15 × USDJPY** — matrix PF 1.35, OOS +RM318 (6/8 folds).
- **CBE15 × NZDJPY** — matrix PF 1.87, OOS +RM252 (5/6 folds).
- **IRE15 × AUDUSD** — matrix PF 1.98, OOS +RM114 (7/8 folds).
- **IRE15 × USDJPY** — matrix PF 3.19, OOS +RM79 (6/6 folds).

All 6 on 0.5× probation. Live methods `_engine_cbe15` (H1 compression → M15 momentum+entry) and `_engine_ire15` (ire_logic on M15), wired into `_dispatch_engine`. **To revert:** remove the CBE15/IRE15 routing/whitelist/probation entries + NZDJPY from config.py, remove the two `_engine_*15` methods + dispatch branches from agent_kira.py, `git checkout v17` for tracked files. Held: IRE15×EURCHF (net-positive but only 4/7 OOS folds).
