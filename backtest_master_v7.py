...
print("=" * 72)
print("  FX COMMAND AGENTS — MASTER BACKTEST v7")
print("  EXPANDED SYMBOL UNIVERSE + COMPATIBILITY MATRIX")
print("  AUDUSD|EURUSD|EURJPY|GBPUSD|USDJPY|GBPJPY|NZDUSD|USDCAD")
print("  XAUUSD → GVE | All others → CTE/MRE/CBE/HPE by regime")
print("=" * 72)
...
CONT_SYMBOLS = list(CTE_PROFILES.keys())
GVE_SYMBOL   = "XAUUSD"
ALL_SYMBOLS  = CONT_SYMBOLS + [GVE_SYMBOL]
...
MRE_SYMS = list(MRE_PROFILES.keys())
...
CBE_SYMS = list(CBE_PROFILES.keys())
...
HPE_SYMS = list(HPE_PROFILES.keys())
...
for sym in ALL_SYMBOLS:
    ...
    if d1 is not None and h4 is not None and h1 is not None:
        data[sym] = entry
        extras = f" {len(m15)}×M15" if sym==GVE_SYMBOL and m15 is not None else ""
...
print(f"  XAUUSD | London Open only | H4 slope filter | BUY only")
print(SEP)
...
print(); print("="*72)
print("  MASTER BACKTEST v7 — FULL REPORT")
print("  Expanded Symbol Universe + Compatibility Matrix")
print("="*72)
...
for sym in all_syms:
    if sym == "XAUUSD": 
        print(f"  {sym:<10} {'GVE':>12} {'—':>12}  Gold-only engine")
        continue
...
df.to_csv("backtest_master_v7.csv",index=False)
with open("backtest_master_v7.json","w") as f:
    json.dump(ALL_TRADES,f,indent=2,default=str)
print(f"\n{SEP}")
print(f"  Saved: backtest_master_v7.csv + backtest_master_v7.json")
