...
| IOE (from v14 draft) | **Superseded (6 Jul 2026, Mahesh)** by IRE — Imbalance Rebalance Engine, the v15 flagship: compression → large displacement → inefficient pricing (FVG) → partial rebalance → continuation entry ON the rebalance. Session-agnostic by design (the phenomenon is not session-bound), session-TAGGED by discipline (the matrix decides where it pays — GVE's London-vs-NY split proves edges are session-lumpy even when logic isn't). Gap analysis: CBE enters the displacement, CTE uses FVGs only inside full HTF-trend context; nobody enters the rebalance — the highest-RR point of the sequence. Design requirement: same-symbol/same-day CBE-overlap tagging so revalidation measures incremental edge vs cannibalization of CBE's entry. Runs on the existing FX universe + whatever v14 promotes; no new market plumbing. **v15 acceptance benchmark (Mahesh, 6 Jul 2026):** walk-forward the full v15 system and report profit vs the v12 core (the original 20 combos) on SAME-DAY data — three lines in VERSION_HISTORY: v12-core OOS net / full-v15 OOS net / delta, plus Monte Carlo risk side-by-side. The v12-core subtotal is already computed in every run, so the comparison is same-dataset by construction. |
| SCE — Session Continuation Engine | **Approved for v15 (Mahesh, 6 Jul 2026) — v15 = IRE + SCE.** Born from SRE's audited failure: fading Asian-range breaches lost PF 0.92 over 2,290 trades, evidencing that those moves CONTINUE. SCE joins the breach at London/NY with continuation levels (measured-move/breakout targets — NOT a sign-flip of SRE's reversal levels; new sre_logic-style level function with TDD). Reuses SRE's audited detection scaffolding (pools, session gates, ATR regime, state isolation) — the cheapest high-prior engine test available. Frequency ceiling: ~48 setups/month across 11 pairs pre-filter, the largest signal-flow lever on the table. Honest caveat recorded: inverting a loser does not automatically clear spread; SCE passes the same §8 bar or gets benched beside SRE. Both v15 engines are continuation-side, share one revalidation run and the v12-vs-v15 profit benchmark. |
