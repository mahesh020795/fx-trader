# ════════════════════════════════════════════════════════════
#  WALK-FORWARD VALIDATOR v11 — Institutional gold standard
...
#
#  Usage: python walkforward_v11.py   (needs backtest_master_v10.json)
# ════════════════════════════════════════════════════════════
import json, sys
...
def load_trades(path="backtest_master_v10.json"):
    with open(path) as f:
        trades = json.load(f)
    for t in trades:
...
def run():
    ...
    print("=" * 70)
    print("  WALK-FORWARD VALIDATION — v11")
    ...
    if pos / max(1, len(fold_results)) >= 0.7 and total > 0:
        ...
        print("  VERDICT: ⚠️ MIXED — positive overall but inconsistent folds.")
...
if __name__ == "__main__":
    try:
        ...
        print("backtest_master_v10.json not found — run backtest_master_v10.py first")
        sys.exit(1)
